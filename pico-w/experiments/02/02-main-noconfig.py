...
# --- Configuration ---
# Pins
ADC_PIN = 26            # GP26 / ADC0 / A0
DIGITAL_SENSOR_PIN = 18 # optional digital out from sensor
RELAY_PIN = 15          # GP15 controls relay IN

# Digital sensor polarity: set to True if `1` means WET, False if `1` means DRY.
# Many digital modules drive HIGH when wet; set to False to invert logic.
DIGITAL_WET_HIGH = True

# Relay polarity: set to True for active-LOW (IN=0 energizes) modules.
# Most 5V relay boards are active-LOW; set to False if yours energizes on HIGH.
...
START_WATER_PERCENT = 30   # start watering when percent <= this
STOP_WATER_PERCENT = 60    # stop watering when percent >= this

# Startup & safety
BOOT_DELAY_SECONDS = 8            # ignore automatic starts during initial boot
REQUIRED_CONSECUTIVE = 4         # consecutive low readings required to start
...
MAX_WATERING_TIME = 3     # maximum continuous watering time
MIN_INTERVAL_BETWEEN_WATERING = 30  # min seconds between watering sessions

# Loop timing
SAMPLE_INTERVAL = 2.0      # seconds between ADC samples

#MOVED TO CONFIG.PY 
# Wi-Fi / Telemetry (disabled by default)
# Set `ENABLE_WIFI = True` to enable telemetry (you must also fill `WIFI_SSID` and `WIFI_PASSWORD`).
ENABLE_WIFI = True
TELEMETRY_INTERVAL = 60    # seconds between telemetry posts
# Fill these with your network credentials on device before deploying
WIFI_SSID = ''
WIFI_PASSWORD = ''
# Example: 'http://192.168.1.120:5000/pico/sensors' - replace with your Pi IP
TELEMETRY_URL = 'http://192.168.1.120:5000/pico/sensors'
